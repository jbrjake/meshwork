...
**Position: next = 0.1.** (Bootstrap B0–B4 done 2026-08-04: crate + gate + TRACE seeded; fixture corpus committed with `fixtures::corpus_covers_features` keeping it honest; stub `gh` records/replays/refuses-mutations (exit 64 unknown, 65 mutation, 66 no opt-in); gate §4 coverage live.)
...
|---|---|---|
| 0.1 | Task-file parser: strict serde model, frontmatter + `## log`/`## comments` tail sections, bullet+continuation comment format; unknown keys warn; parse failure → `invalid` row carrying filename-recovered ID (A1, A6, I2, K1) | `cargo test parse::` |
| 0.2 | ID generation: `<alias>-<4-char base32>`, collision re-roll against local files; seedable RNG hook for tests (A4) | `cargo test id::` |
