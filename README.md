...
sa-pdk6
  docs/meshwork/sa-pdk6-reproduce-the-600m-row-spill-cliff.md

$ cat docs/meshwork/sa-pdk6-reproduce-the-600m-row-spill-cliff.md
---
id: sa-pdk6
...
```
$ meshwork add "Fix spill batch sizing" --cat engine/spill --needs sa-pdk6 --seq 10 --verify "cargo test spill::batch"
sa-vj0d
  docs/meshwork/sa-vj0d-fix-spill-batch-sizing.md
$ meshwork add "Write the spill postmortem" --cat docs --verify "test -f docs/postmortem.md"
sa-6r27
  docs/meshwork/sa-6r27-write-the-spill-postmortem.md
```
...
sa-pdk6: comment added as [claude]
...
```
$ meshwork set sa-vj0d --handoff "Cliff is governor wakeup, not batch size — don't burn a session re-deriving that (comment on sa-pdk6 has the repro). Try wakeup=250ms before touching batch math."
sa-vj0d handoff set
```
...
A task is one markdown file. This is the entire storage format — `sa-vj0d`, the blocked task from the session loop, verbatim:
...
```bash
echo "v0.1.2" > .meshwork-version     # commit this
