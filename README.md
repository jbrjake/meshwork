...
| context replay ("turned tokens") — tracker content rides the context window, so it is re-submitted on **every subsequent turn**, and re-billed at cache-write rates whenever the cache has gone cold | resident tracker content replayed across ~154 turns/session: **4.16M tokens** — 10.5% of the session's 39.7M-token total replay | **0.49M tokens** — 1.5% of 32.2M — **8.5× less**, and the saving compounds with session length: late turns that add 2 tokens still re-pay everything resident before them |
...
*Busywork counted from the session transcripts by `scripts/admin-tokens.py`: every tool call (input + result) whose command or path touches the tracker — TODO.md/HANDOFF reads, edits, and cap-check thrash before; every meshwork CLI call, store file edit, and console todo mirroring after — at 4 chars/token. The classification errs against meshwork: its whole CLI surface counts as busywork. Context replay measured by `scripts/turned-tokens.py` from the per-request usage records the API reported (cache reads + cache writes + uncached input, priced per model at real cache-read/write rates): tracker-resident tokens × every subsequent turn. Whole sessions cost ~$27.60 before vs ~$25.47 after — replay is dominated by code and tool results either way; the tracker's share of it is what shrank 8.5×.*
