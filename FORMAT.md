...
`actor` is populated from the only two places the store records who: comment authors and `claimed by <author>` transition notes. Nothing else attributes — a `→done` line carries a sha, not a person.
