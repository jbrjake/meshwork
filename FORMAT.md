...
| `covers` | list of `{ref, sha}` | clause pins (MW-T1): `ref` is a clause ref — `path#sp-<slug>` or `repo#path#sp-<slug>`, the `docs:` document spelling with a fragment naming a clause anchor — and `sha` is the SHA-256, lowercase hex, of that clause's text when the pin was written; see *Clause pins*. Written by `cover`; a bare ref or an entry without a well-formed `sha` parses but is a lint error on a live task (MW-T3). Never an edge: the target is a clause, not a task |
...
A spec document opts into traceability one clause at a time (MW-T1–T3, MW-T6): a heading that ends with `{#sp-<slug>}` — the slug lowercase letters, digits and hyphens — names a **clause**, which is the section under that heading up to the next heading of the same or a shallower level, headings inside fenced code not counting. Anchor ids are never renumbered or reused. The clause's **text** is that section without the heading line, every line right-trimmed, leading and trailing blank lines dropped, lines joined with `\n`; its **hash** is the SHA-256 of that text, lowercase hex. Because the heading line is outside the text, retitling a heading neither breaks a ref nor moves a hash (MW-T2); any change to the words below it does.

A **clause ref** is `<doc>#sp-<slug>`, where `<doc>` is a `docs:` document spelling — a repo-relative path, or `repo#path` for a registered sibling — resolved and confined exactly as a `docs:` link is. It is not a `docs:` ref: the fragment names an anchor id, never a heading slug, so the two spellings cannot be confused. A **pin** is a ref plus the hash of the clause as read from disk when the pin was written — no version control is needed to detect that a clause moved; what version control would add is who moved it (MW-T6). The reference binary writes pins with `cover <task> <ref>` and re-reads them with `--repin`; readers project every pin as a `covers` row and MAY compare `sha` with the clause's current hash to report drift.
...
The projection is seven tables. `repo` is the registry name from the portfolio's `repos.toml`, defaulting to the repo directory's name; `gid` is `repo#id` and is unique across a loaded set:
...
| `covers` | `gid`, `ref` (the clause ref as written), `sha` (the pinned hash; NULL when the entry carries none), `resolved` (the ref names a clause on disk right now, read through the registry for a `repo#path` ref) — one row per `covers:` entry (MW-T1); see *Clause pins* |
