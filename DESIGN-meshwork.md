...
**The DSL (parse stage, landed via mw-sascrgs).** A `verify:` that leads with a DSL keyword parses as predicates — `exists <path>` / `absent <path>` / `contains <path> <lit|/regex/>` / `run <runner> <args…>` / `all(p, …)` — in `src/verify_dsl.rs`, golden-pinned by `fixtures/verify-dsl/corpus.txt`. Safety is by construction, not sanitization: per-runner argv grammars (today `cargo test|build|fmt`), tight character classes, no leading dashes anywhere (argument injection, per the Cursor/Flowise prior art), repo-relative traversal-free paths; shell metacharacters are just characters that fail a class. Keyword-led text that doesn't parse REFUSES loudly — it never downgrades to legacy shell, because that silent downgrade would reopen the drive-by hole. Non-keyword text stays legacy shell behind the gate above; nothing in the parser runs anything. Execution (mw-dthxs3q, `src/verify_exec.rs`): `exists/absent/contains` evaluate natively without a process; `run` spawns its argv array directly — never a shell — env scrubbed to PATH/HOME/CARGO_HOME/TMPDIR, cwd = repo root, 300s wall clock, 256KB output cap. Gate routing (owner directives 2026-08-14). Native predicates (`exists/absent/contains`) run without the trust gate — pure reads, no code loads. `run` also executes without approval — frictionless test-backed verifies are the point of the DSL — but only under the ride-along guard: a `run` verify is auto-trusted iff every commit that introduced or modified its task file touched nothing outside `docs/meshwork/`, judged over the full delta of the merge that landed each commit (a task authored in this clone and not yet committed is the operator's own and passes). A task that arrived alongside code changes has its `run` gated exactly like legacy shell — the ride-along test/`build.rs` is the drive-by payload, and argv safety pins which program starts, not what code cargo loads. Accepted residual, ruled with eyes open: landing the test in one merge and the task in a later one defeats the guard — that costs the attacker two operator-accepted merges instead of one; tree-hash bless (mw-hz1ezcg) is the belt-and-braces escalation if that residual ever bites.
