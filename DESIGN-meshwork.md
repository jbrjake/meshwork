...
| `add "title" [--cat p] [--label l] [--needs id..] [--parent id] [--from id] [--verify cmd] [--seq n] [--docs link..] [--dry-run]` | create task file, print id; missing `--verify` = lint warning until set (MW-E2); `--dry-run` prints the would-be file, writes nothing (mw-0wvndqa); `--category`/`--doc` are hidden aliases of `--cat`/`--docs`, and curated unknown verbs (log→comment, done→close, rm→drop) fail with a two-line did-you-mean (mw-5hrb22q) |
