// mw-n6nvzpa: store format version marker. init writes the format this
// binary understands (2 since archive bundles, mw-bvxpeef); absent means 1
// (every pre-marker store); a format newer than this binary knows is
// refused loudly — semantic changes need detection, and that is
...
fn format_marker_newer_is_refused_loudly() {
    ...
    meshwork(&repo)
        ...
        .assert()
        .failure()
        .stderr(predicates::str::contains("format 3"))
