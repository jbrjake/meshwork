...
the pulse row per store, a probe per public view, and the pooled hazard and backlog-day tables.
    ...
    python3 scripts/mine_views.py --sql      # the views as CREATE VIEW statements (the proposal's Appendix A)
...
V["clock"] = "SELECT now() AS now, CAST(now() AS DATE) AS today, 'system' AS source"
V["p_win"] = f"SELECT c.now - INTERVAL '{WINDOW} days' AS since, c.now FROM clock c"
...
SELECT t.gid, t.repo, t.id, t.title, t.status, t.category, t.seq,
       ...
       (to_unixtime(coalesce(tr.terminal_at, c.now)) - to_unixtime({ts('t.created')})) / 3600.0 AS age_h,
       ...
       (to_unixtime(tr.done_at) - to_unixtime({ts('t.created')})) / 3600.0 AS cycle_h,
...
def main():
    ...
    if "--check" in sys.argv:
        if not out or failures:
            print(f"check: FAIL — {failures or 'no stores'}")
            return 1
        print(f"check: ok — {sum(len(p) for p in out.values())} probes, 0 errors")
    return 0
