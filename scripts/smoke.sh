...
# Fast unit tier only (unit tests live in src/ modules of the library; the binary's harness
# rides along). No RUSTFLAGS here: flipping flags invalidates the whole dep cache per commit;
# -D warnings is gate §2's job.
OUT=$(cargo test --lib --bins 2>&1); RC=$?
if [[ $RC -ne 0 ]]; then
...
# A tier that ran nothing proved nothing — the vacuity the verify DSL refuses for
# `run cargo test` holds here too.
[[ $N -gt 0 ]] || { echo "smoke: FAIL 0 tests ran — the fast tier is vacuous"; exit 1; }
