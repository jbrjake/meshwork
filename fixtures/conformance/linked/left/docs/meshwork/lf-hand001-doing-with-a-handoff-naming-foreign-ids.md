...
makes `pulse.handoff_stale_n` 1 for `left`. The minted `handoff by` line
is an `events` row of kind `handoff` with an actor, and the exact bound
for the handoff mentions' `moved_since_activity`. The comment below names
a foreign id with a stamp, so its `first_at` is set.

## log
- 2026-08-03 created
- 2026-08-05T08:00Z open→doing — claimed by claude (session_left)
- 2026-08-05T08:30Z handoff by claude (session_left)
