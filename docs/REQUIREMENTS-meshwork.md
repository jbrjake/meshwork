...
- **MW-A1 (MUST)** Canonical store = one markdown file per task with YAML frontmatter, under `docs/meshwork/` in the owning repo's git tree (flat — task files sit beside the store's config; moved from `meshwork/tasks/` by owner ruling 2026-08-06, mw-acgp; pre-move stores migrate with a `git mv`, nothing else). Human-readable, hand-editable in any editor; the tool MUST tolerate and re-validate hand edits. (Backlog.md's concept, our schema.) Terminal tasks (done/dropped) live under `docs/meshwork/archive/`, moved automatically by the lifecycle verbs and always loaded — queries and edge resolution are location-blind (mw-45e2qf4). The archive MUST NOT grow one file per closed task forever: past 100 loose archived files `lint` warns and `lint --fix` concatenates them into `archive/bundle-NNNN.md` (plain task-document concatenation, ≤ 512 KiB per bundle; the store's format becomes 2), a bundled task stays loaded, queryable and editable through every verb, and `reopen` splits it back out into a file of its own (mw-bvxpeef, owner request 2026-08-06; FORMAT.md §Bundles).
...
- **MW-E2 (MUST)** Every task carries a `verify:` command (set at creation via `add --verify` or by hand-edit; missing `verify:` is a lint warning while open and forces `close` to demand `--waive`); `meshwork close <id>` runs it via `sh -c` from the repo root, records exit code + date in the log, and closes only on exit 0. `--waive "<reason>"` is recorded, loud, and queryable. Can't write a verify → the task isn't ready; split out a spike whose deliverable IS the verify (baseline rule, preserved verbatim). Amended by owner ruling 2026-08-07 (mw-6wdpz1b): capture without a `verify:` stays legal, but `start` MUST refuse it (writing the done-test is the first unit of the work) and `ready`/`prime` MUST surface the gap loudly (`needs-verify`); `--waive` semantics are unchanged. Amended by owner ruling 2026-08-14 (mw-4aqmf0t): the preferred `verify:` form is the DSL (DESIGN §12b) — parsed predicates executed argv-style, never `sh -c`; `run cargo test` MUST observe `ok. N passed` with N ≥ 1 in the output, so a filter matching nothing can never close a task; legacy shell text stays legal forever behind MW-E5 and wears lint's `verify-shell` warning as migration pressure.
