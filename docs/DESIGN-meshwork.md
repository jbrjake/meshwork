...
verify: run cargo test spill::cliff_600m   # DSL preferred (§12b); shell text stays legal, gated
docs:
...
Addressed tasks (mw-hfvtx0s, owner lane 2026-08-17): `to: <repo|repo#id>` marks a task as an *ask* aimed at another repo; `answers: <gid>` marks a task as its answer. Both are plain frontmatter — no authoring verb, no flag (the `relates:` precedent) — and delivery is a read-time join, never a transport: the addressee's prime/ready scan the portfolio union for non-terminal asks addressed to them that no non-dropped task answers. Nothing is written into the addressee's store (the merge stays the trust unit, mw-egksvhm), an absent sender repo contributes nothing, and `answers` edges never gate ready. This is the one sanctioned full-union read inside single-repo verbs — mw-k7r5's direct-lookup discipline still governs dep resolution, but incoming asks are unknowable in advance, and the union costs ~30ms cold (MW-C4). Normative reader semantics live in FORMAT.md §Task file.
...
|---|---|
| `tasks` | `gid` (`repo#id`), `repo`, `id`, `title`, `status`, `category`, `verify`, `waived` (reason or NULL — makes MW-E2's "queryable" true), `seq`, `created`, `blocked_reason`, `claimed_by` (advisory claimant or NULL, mw-tb6gdr9), `github`, `addressed_to` (the `to:` key or NULL, mw-hfvtx0s), `path`, `body` (the description section, trimmed; `''` when parsed-and-empty, NULL on invalid rows — mw-getx732), `handoff` (the block text or NULL, mw-5xdyxep), `parent` (the key as written or NULL — convenience projection of the one child-points-up edge kind, `edges` stays normative; mw-0ssk8dg — same task made the `q` error path enumerate the queryable tables) |
...
| `add "title" [--cat p] [--label l] [--needs id..] [--parent id] [--from id] [--verify cmd] [--seq n] [--docs link..] [--body "text"\|@file\|-] [--dry-run]` | create task file, print id; missing `--verify` = lint warning until set (MW-E2); `--body` lands the description at creation, above the tail sections, per the mw-rz4ey2h prose spellings (mw-s3905fv, §6 ruling 2026-08-21 — every substantive pilot task went add-then-shell-append, the stray-tail damage source; empty payload writes no block); `--dry-run` prints the would-be file, writes nothing (mw-0wvndqa); curated unknown verbs (log→comment, done→close, rm→drop) fail with a two-line did-you-mean (mw-5hrb22q); `--category`/`--doc` are hidden aliases of `--cat`/`--docs` on add and set (ruling 2026-08-10, mw-42ygb52); `--verify` names a predicate, not a shell line — keyword-led text that does not parse is refused at authoring (mw-8e769q0, see `verify`); a same-repo `--needs`/`--parent`/`--from` target that does not exist is refused (mw-tkgvsdz: a `--from` typo minted two dangling edges silently), a cross-repo target the registry knows no repo for, or resolves to a store without that task, is warned about, a `--docs` link that dead-ends is warned about at add rather than at the next lint, and inline `--body` text carrying a backtick or `$(` draws a warning pointing at `@file`/`-`; `add --batch` runs the same checks per document, atomically, with the batch's own handles as known ids. The did-you-mean table also covers the inbox guesses — `next`/`inbox`/`addressed`/`list`/`help` point at `prime`, `ready`, `--help`, never at a writing verb; `portfolio show` says show is single-repo — and frontmatter-only flags (`--to`/`--answers`/`--relates`, and `--body`/`--from`/`--parent`/`--needs` off `add`) name the open door instead of clap's `-- --flag` tip (mw-48mzck9) |
| `add --batch <file\|-> [--dry-run]` | several tasks atomically (mw-af4kbjy): concatenated §2 documents, `id:` omitted, local-only `handle: <name>` — `@name` legal anywhere an id is (needs/parent/from/relates/answers); ids minted, refs rewritten, all files or none; unknown frontmatter keys refuse the whole batch, and `from:` is an input alias rewritten to the canonical `discovered-from:` (mw-16pyc5g); each document's verify records the authoring clone's approval at write (approve-at-mint, §12b as amended 2026-08-24, mw-9dq6850); `--dry-run` prints the would-be files, writes nothing |
| `set <id> [--seq n] [--docs link..] [--handoff "text"\|@file\|-] [--cat p] [--verify cmd] [--title "t"]` | field edits without opening the file (mw-0f4j); `--docs` appends, `--handoff` replaces the block — `@file`/`-` (stdin) spellings land prose without transiting shell quoting (mw-rz4ey2h, ruling 2026-08-10), and an empty payload drops the key entirely, never a dangling `handoff: |` (mw-gbep3j8); `--cat`/`--verify`/`--title` grown by owner ruling 2026-08-10 (mw-f1x71yg — nine pilot sessions hand-rewrote files for these) — retitling never renames the file (the slug is cosmetic), and a replaced `verify:` records the authoring clone's approval at write (approve-at-mint, §12b as amended 2026-08-21) while re-arming every other clone's gate by construction |
| `show <id> [--docs] [--comments]` | full task; last-3 comments by default (MW-K4); `--docs` = anchor-scoped excerpts, capped ~4KB/link (bytes, MW-D5/F2); `commits:` tail derives the closing work from `git log --grep=<id>` (id-in-subject convention, local refs only, retroactive — mw-ntn0t32; close also anchors HEAD into the `→done` note); stranded tail content surfaces as an in-body marker counting the ignored lines, placed where they would have rendered — stderr warnings scroll past unread (mw-7tseswy, same counter as lint's stray-tail-content); `file:` is the path the store holds, `archive/` included (mw-7ywrxf1); an id with a sibling's prefix, or a `repo#id`, is not "not found" when the registry resolves it — the refusal names the repo and prints the one-liner `(cd <path> && ./docs/meshwork/meshwork show <id>)` (mw-48mzck9: nine such misses, each an id prime had just printed) |
...
| `start [--as <author>] / block --reason / drop [--reason "text"] / reopen <id>` | status transitions + log line; `drop --reason` is optional and lands on the →dropped log entry like block's (mw-bd390q6, §6 ruling 2026-08-22 — two pilot sessions guessed the block symmetry existed and lost the rationale to chat scrollback when it didn't; bare drop stays legal, terminal drops are sometimes self-evident); `start` refuses a task with no `verify:` — needs-verify; writing the done-test is the first unit of the work (mw-6wdpz1b, amending MW-E2) — and red-checks the verify before flipping, routed like close (§12b): native DSL runs untrusted (pure reads), DSL `run` needs trust or store-only provenance, shell needs the gate; already-green or exit 127 draws a loud advisory warning, gated text never runs and the skip says so (mw-175bn4c, the mw-6wdpz1b sister — warning not refusal per the mw-kkvs8zq no-new-surface precedent); a red-check that spawns anything — DSL `run` or approved shell — is announced first (`note: red-checking <id>'s verify — may build; up to Ns`) and runs under the executor's wall clock (`MESHWORK_RUN_TIMEOUT` seconds, else five minutes) with its output capped, a timeout reported as a warning that still transitions (mw-82thxwz: an unscoped `cargo test` compiled for minutes in silence, and the agent hand-flipped the status) — and records an advisory `claimed-by:` when the MW-K1 chain resolves an identity (no identity = no claim, never an error); close/drop/reopen release the claim, block keeps it (mw-tb6gdr9); close/drop also strip `handoff:` — the voice belongs to what's up next, and left behind it becomes handoff-stale on an archived file (mw-e8hg2kt); `reopen`: blocked\|doing\|done → open (the missing inverse — without it every unblock is a hand-edit); `drop` additionally warns on live inbound cross-repo needs when the registry resolves (mw-kkvs8zq, §9 — advisory stderr lines, the drop always proceeds; unscannable repos reported, never guessed) |
| `close <id> [--waive "reason"] [--approve]` | run `verify:`, close on a pass only (MW-E2), routed by shape (§12b, mw-4aqmf0t): DSL native predicates run ungated (pure reads); DSL `run` runs approval-free only on store-only provenance (mw-egksvhm), otherwise it gates like shell with the refusal naming the arrival to review; malformed DSL refuses outright — never runs, never gate-prompts; legacy shell sits behind the MW-E5 trust gate — `--approve` shows the text and records this clone's approval, `MESHWORK_TRUST=1` is the reviewed-checkout grant (§12b, mw-9rc4vs6); `run cargo test` demands an observed `ok. N passed`, N ≥ 1 — a filter matching nothing can never close a task |
| `verify <id>` | run the task's `verify:` and report the verdict — close nothing, release nothing, write nothing, not even close's attempt line (mw-dx4pndb, §6 ruling 2026-08-22): red-first authoring and rot-sweeps had no supported path — leras hand-rolled the same extraction loop six times across ~55 executions, in the *interactive* shell, the authoring-shell mismatch that shipped 28 fail-closed rg-verifies; routing, gates, and refusals are exactly close's (§12b) via the shared route — any status runs (a rotted verify on a done task is the sweep's quarry); no `--approve` (approval stays a close-side act; the refusal names it) and no `--all-open` sweep (single-task only — a sweep waits for its own evidence and its own ruling). `verify --help` and `close --help` print the DSL grammar; a keyword-led verify that does not parse is refused where it is authored — `add --verify`, `set --verify`, `add --batch` (nothing written) — and `start` refuses to open work behind one instead of warning and transitioning, each refusal carrying the one-line grammar (mw-8e769q0: `add --verify 'run cargo test -p leras topn'` minted, and only close said no) |
| `dep add / dep rm <a> --needs <b>` | edge edits without opening the file; a target typed positionally (`dep add A B`, typed in two sessions a week apart) is refused with the flagged form to copy, and the success line models the flag (`A --needs B added (now: […])`) — mw-48mzck9 |
| `ready / blocked / tree / why` | §5 |
| `q "SELECT …" [--json]` | raw SQL (MW-C1); `q --help` lists every queryable table with its columns in projection order (`tables::SCHEMA`, pinned to the registered Arrow schemas by a unit test) and says that `--json` nests rows under `data.rows`; a failing query names the tables and points at `q --help` for the columns (mw-myas0dd — two sessions reverse-engineered the column list with `SELECT *`); a local query on `addressed_to` that returns no rows notes that the column is outbound here and the inbox is `prime`/`ready` or `portfolio q` (mw-48mzck9) |
...
| `prime` | §7 |
| `lint [--fix]` | schema, cycles, anchors, attachment size, post-merge damage (MW-I2/K3); verify hygiene on live tasks (§12b): `verify-trivial` (mw-221f3jt), `verify-shell` (legacy shell — migration pressure, mw-4aqmf0t), `verify-malformed` (will refuse at close), `verify-changed-since-approval` (approved-vs-current diff, mw-yyf1bab; prime carries the ids as a nudge), `verify-path-missing` — a live task's `contains <path>` or plain legacy `grep … <path>` names a file absent from the tree, so it can never close and reads as unfinished work; `exists` is excluded by definition, escapes are `path-escape`'s (mw-c3s9209, doc-missing's twin on the field that decides closability); tail hygiene: `stray-tail-content` on live tasks, relocated by `--fix` (mw-t01ek6s/mw-n3xgfs0, FORMAT.md Stray content), `fence-unclosed` on every task — an open fence swallows the tail sections, no `--fix` (mw-gw569q7); doing-rot (mw-06j1wqe): `doing-stale` past the activity window, `doing-unclaimed`; `status-unlogged` — a terminal status with no matching transition log entry, the hand-flip signature (mw-nfv26ss); `path-escape` — a docs:/attachment path resolving outside the repo, never read (mw-2pz0zqc, §12b); `doc-repo-unknown` — a `repo#path` docs link naming a repo the registry does not know (mw-8q0srvb; a link the session cannot resolve for want of a registry or checkout is unverifiable, not a finding) — warnings all. `--fix` rewrites a `../<dir>/rest` docs link to `<repo>#rest` when the registry resolves `<dir>`, logged. `--fix` also mends indented lines stranded under a scalar key after a blank line — what a block strip that stopped at the blank left behind (mw-87gxe7q, FORMAT.md Block values) — and names every file still unparseable after its pass as `cannot repair <id>: <reason>`, never silence |
...
1. **headline** — counts + category rollup capped at top 5 groups (group by first two category segments; rank by min seq among open members — seq is the priority primitive, there is no priority field; rest collapses to `… +N`, MW-D2 pattern). When the inbox (2b) is non-empty the counts line ends `· N asks unanswered, oldest Dd` (mw-r6g9bhe): an ask's age is `created` plus the absence of an answer, so silence is a number in every session's first line, whatever the budget cuts below it.
...
2b. **addressed** (mw-hfvtx0s, when any): incoming asks from the read-time join (§2), top 3, each row suffixed with its age `(Dd)` — informs the session before it commits to a task, never displaces next. An ask whose `to:` names no registered repo is warned about at `add --batch` time, the one moment the author is listening (mw-r6g9bhe).
...
New frontmatter key `handoff:` (multi-line block): the outgoing session's color commentary to the incoming one — the ONLY authored piece of the view, meaningful solely on up-next tasks. Rewritten freely (history belongs in comments); set via `meshwork set <id> --handoff` or hand-edit — both legal (mw-0f4j superseded the original hand-edit-only ruling, owner 2026-08-06). Lint warns when `handoff:` sits on a done task; `close`/`drop` strip the block at the terminal transition (mw-e8hg2kt), so the warning marks hand-flips and merge arrivals, never a CLI close. Session-end ritual: refresh `handoff:` on whatever is up next; comment anything history-worthy. Landing commit: delete docs/HANDOFF.md; drop CLAUDE.md ritual step 4's HANDOFF clause + baseline override 2; adoption-skill step 4 becomes "delete HANDOFF.md"; move HANDOFF's decisions line to §15 and its 2.3 re-bless note into mw-k7r5's body.
...
Sequence hygiene (mw-2nmsys2): `sequence.md` is authored, denormalized cross-repo state, so the same registry-aware lint pass (same env-opt-in trigger — no §6 delta) warns `dangling-sequence` when an entry resolves nowhere it could: an unregistered repo name, or a registered, locally-present store holding no such id. An absent repo is unresolvable — the skipped-repo notice's business, never guessed (MW-G5) — and an entry resolving to done/dropped is satisfied: prune's business, not damage. A warning, not an error, because a dangling entry degrades ordering ("first ready one wins" skips it), never readiness semantics. Satisfied entries never linger either — autoprune (mw-chcqk6g, owner-ruled 2026-08-10: no `--prune` flag, running any `portfolio` verb prunes): before querying, entries resolving to done/dropped in a registered, present repo are removed from sequence.md — the clutter archive/ already solves for task files, solved the same way for the overlay. Headings, prose, live, dangling, and unresolvable entries survive byte-for-byte (absence is not evidence of death, MW-G5; dangling stays lint's finding). Removals are reported — stderr in text mode, a `pruned` list beside `skipped` in JSON data — and the file is versioned in the portfolio repo, so git diff is the review surface and the undo. A present sequence.md that cannot be read or written back is a loud error.

Drop across the boundary (mw-kkvs8zq): only done/dropped satisfies a dependency, and drop is the one that means the needed work never happened — inside a repo that's survivable (same person, same head), but across repos it silently unblocks tasks nobody on either side is looking at. `drop` therefore scans registered present repos for live inbound cross-repo `needs` on the dropped id when the quiet chain resolves a registry, and prints each as an advisory stderr warning, one per line, with unscannable repos (absent checkout, broken store) reported as unscanned — the scan is advisory, so sibling-repo damage never blocks a local lifecycle verb, while a found-but-broken *registry* stays the mw-k7r5 loud error, fired before anything is written. Refusing the drop outright would be a §6 question — not taken; the warning is behavior, no new surface.
...
**Threat.** Task files are data that arrive through git merge — a PR, a fetch, a synced clone. Anyone who can land a file in the repo controls every field in it, and `verify:` is the one field the tool ever *executes* (`sh -c` from the repo root, at `close`). The drive-by path: a merged task carries `verify: curl … | sh`; a later operator — human, or an agent closing tasks in a loop — runs `meshwork close <id>`; arbitrary code executes with that operator's credentials. The DSL-era variant (ruled 2026-08-14): one PR carries a task plus the test its `run` verify names — the task self-verifies against attacker code at close, arbitrary execution AND attacker-defined success criteria, with no shell metacharacter anywhere in the verify. The append-only mirror, agent-driven sessions, and portfolio-wide adoption all widen exposure: `close` runs in more places, read by fewer eyes.

**Execution points.** Two: `close` (MW-E2), and `start`'s advisory red-check (mw-175bn4c) — which only ever runs text this clone already trusts under the gate below, so it widens no exposure. `mirror` invokes `gh` with fixed argv — task content travels as arguments and stdin, never through a shell — and stays that way by rule. Adjacent surfaces: repo-relative path resolution (`docs:`, attachments, DSL paths) is confined by `src/paths.rs` (mw-2pz0zqc) — lexical refusal of absolute/`..` paths plus a canonicalize-when-exists comparison, so symlink escapes refuse too; every read site (`show --docs`, lint's doc and attachment checks, verify execution) joins through it, refusals are loud, and lint carries `path-escape`. Terminal escape sequences in rendered task content strip at render time (mw-8fmsws3): `cli::sanitize` drops C0/C1 controls (keeping `\n`/`\t`) at every text-mode seam — show, prime (which is also the hook-injected agent context), ready, why/blocked, search, lint findings, `q` cells — render-only, files keep bytes as written; JSON needs nothing (serde escapes by construction), and frontmatter can't smuggle controls at all — YAML forbids them raw, so such a file parses INVALID, loud and inert; the CLI also refuses to mint one (mw-3tzfqmq): control characters in frontmatter-bound verb input (add/set fields, block/drop reasons, the resolved claimant) refuse at the verb before any write, keeping `\n`/`\t` legal in genuinely multi-line prose (handoff, body) — a success report over an unparseable row was the bug. This ruling is about code execution.

**The boundary (ruled).** Trust attaches to the *exact verify text, per task, approved by the operator of this clone* — trust-on-first-use, the direnv-allow pattern. Approvals are `id TAB text` records in the gitignored `.cache/` (mw-9rc4vs6; plaintext since mw-yyf1bab — hash-era lines still gate-pass): per-clone local state that can never arrive via merge, exactly because merged content is what's untrusted. Recording the text, not a digest, is what lets lint and prime surface `verify-changed-since-approval` — approved-vs-current on screen — because a verify silently edited after approval is the weakening attack, and the close-time re-approval prompt is only a speed bump if the operator is clicking through. Git metadata is NEVER a trust signal — author and committer are self-professed strings (same rule as MW-K1 identity), trivially spoofable by the attacker this section is about. A fresh clone trusts nothing until its operator does. Authoring text through this clone's own CLI is approval (approve-at-mint, mw-2kgkn0j/mw-51x0wty, ruled 2026-08-21; extended to batch documents 2026-08-24, mw-9dq6850): `add --verify`, `set --verify`, and each verify carried in an `add --batch` document record the same ledger entry `close --approve` would, at write time — the CLI operator can already execute shell directly, so the prompt gated nothing real for self-authored text (field evidence: every observed refusal across two adopting stores — 6/6, then 5/5 and ~63 `--approve` invocations — was the author rubber-stamping their own just-written text, training the reflex that would neutralize the gate against genuinely merged verifies). Merged-in or hand-edited verify text still prompts — that asymmetry is the point; approvals stay per-clone and never travel.
...
**The DSL (parse stage, landed via mw-sascrgs).** A `verify:` that leads with a DSL keyword parses as predicates — `exists <path>` / `absent <path>` / `contains <path> <lit|/regex/>` / `run <runner> <args…>` / `all(p, …)` — in `src/verify_dsl.rs`, golden-pinned by `fixtures/verify-dsl/corpus.txt`. Safety is by construction, not sanitization: per-runner argv grammars (today `cargo test|build|fmt`), tight character classes, no leading dashes anywhere (argument injection, per the Cursor/Flowise prior art), repo-relative traversal-free paths; shell metacharacters are just characters that fail a class. Keyword-led text that doesn't parse REFUSES loudly — it never downgrades to legacy shell, because that silent downgrade would reopen the drive-by hole. Non-keyword text stays legacy shell behind the gate above; nothing in the parser runs anything. Execution (mw-dthxs3q, `src/verify_exec.rs`): `exists/absent/contains` evaluate natively without a process; `run` spawns its argv array directly — never a shell — env scrubbed to PATH/HOME/CARGO_HOME/TMPDIR, cwd = repo root, 300s wall clock, 256KB output cap. Gate routing (owner directives 2026-08-14). Native predicates (`exists/absent/contains`) run without the trust gate — pure reads, no code loads. `run` also executes without approval — frictionless test-backed verifies are the point of the DSL — under the ride-along guard, a union judged conservatively in both directions: every commit that ever touched the task file is judged by its own full delta — a squash-merge is the whole PR in one linear commit, so single-commit task+code combos always gate — and a commit that reached mainline through a merge is ADDITIONALLY judged by that merge's entire first-parent delta (`M^1..M`), so a PR splitting task and test across inner commits is still one arrival. The merge basis widens judgment to the delivered group; it never exempts a commit, and no merge style is forced on contributors. Any delta touching a path outside `docs/meshwork/` gates every `run` on that task exactly like legacy shell — ride-along code (a test, `build.rs`, a proc-macro) is the payload argv safety cannot see, and a task must never self-verify against code that arrived with it. Store-only history and uncommitted task files pass. Accepted frictions and residuals, ruled with eyes open: a task whose file was ever touched by a code-carrying commit (including this repo's own task+code close-ritual commits) needs one `--approve` on later `run` re-runs; and test-in-one-merge, task-in-a-later-one still defeats the guard — tree-hash bless (mw-hz1ezcg) is the escalation if that bites. When git cannot answer, the guard degrades toward gating, never toward trust. `run cargo test` additionally demands an observed pass (ruled 2026-08-14, mw-4aqmf0t): `cargo test` exits 0 when a filter matches nothing, so the executor requires `ok. N passed` with N ≥ 1 in the captured output before the predicate counts as green — the vacuity the store's old observed-pass shell idiom (`out=$(cargo test F 2>&1) && echo "$out" | grep -qE 'ok\. [1-9][0-9]* passed'`) existed to plug, now native and non-optional. Migration state (mw-4aqmf0t): this repo's store is DSL everywhere the grammar reaches — 43 of 51 live verifies at migration; the holdouts (tool self-invocations, gate scripts, a disjunction) stay legacy shell deliberately. Lint applies the pressure: `verify-shell` warns on every live legacy-shell verify, `verify-malformed` on keyword-led text that will refuse at close — warnings, never errors, because the escape hatch is a feature. v1.x never removes legacy shell; it just makes it loud, gated, and rare.
...
| 9 | README transcript replay (mw-5fekg2q): every `$ `-led fence executes in order against scratch stores (`acme` cargo quick-start staged red-then-fixed at the `...` elision, `demo` workflow store, alias `sa`); pasted lines must appear in the real output in order, ids mapped pasted→minted, timestamps/git anchors normalized, row order enforced; a distinct `MESHWORK_TODAY` minute per command pins tied-seq ordering | divergent transcript — first divergence named by README line number |
...
6. **Determinism hooks are env vars**: `MESHWORK_ID_SEED` (id generation), `MESHWORK_TODAY` (clock), `MESHWORK_BLESS` (golden re-bless). Tests and fixtures depend on them; they are contract, not convenience. (Moved from hand-written HANDOFF at its retirement, 2026-08-06.) Later additions to the contract: `MESHWORK_TRUST` (MW-E5 reviewed-checkout grant, §12b), `MESHWORK_PORTFOLIO` (portfolio-dir override — the only registry trigger for single-repo lint, and the test/nonstandard-machine override for `portfolio` verbs, whose default is `~/Documents/code/portfolio` per §15.4; §9 — mw-mrjccx2, mw-9093), and `MESHWORK_RUN_TIMEOUT` (whole seconds; the wall clock on every verify spawn — DSL `run` at close and both red-check paths at start — in place of the five-minute default, so a test can watch a hang die; mw-82thxwz).
...
12. **Addressed tasks are a read-time join, not a message system** (owner lane commit 2026-08-17, mw-hfvtx0s). `to:`/`answers:` are frontmatter only — no verb, no flag, no transport, no broker, no write into the addressee's store; surfacing derives from the union at read time and an ask un-surfaces the moment a non-dropped answer exists. The §3-REQUIREMENTS fences (no daemon, no notifications, no sync) hold: nothing runs, nothing pushes, nothing mirrors — a reader that never loads the union simply never sees an ask. Substance in §2; reader semantics in FORMAT.md.
13. **Analytics as contract** (owner ruling 2026-09-08, mw-w2920xb — *"Rule yes on all six"*, the six items of `PLAN-proposals-implementation.md` R-A with the plan's recommendations). Thirteen named views plus `clock` are the derived projection, published in FORMAT.md §Views and registered by the binary in query sessions — additive, no format or schema bump, the prose derivation normative and the SQL reference (REQUIREMENTS §S). The pulse renders inside `prime`'s weather, ≤ 5 lines / ≤ 800 bytes, top of the section, equal to `SELECT * FROM pulse` by a gate test. Reads never prune: `q`, `portfolio q` and `stats` leave `sequence.md` alone; only `next`/`ready` prune (narrows mw-chcqk6g). The three §3 scope clauses are recorded there. `set --handoff` mints a log line so a handoff carries an author and an age. The closure convention — `done` is current status, reopen is not a closure, date-only stamps are midnight UTC — is pinned in FORMAT.md, one rule for every reader. What this does not decide: the `stats` verb and every other §6 addition stay with R-D (mw-xg67266); no metric orders the ready set; no metric is a velocity, target or forecast; no generated prose.
